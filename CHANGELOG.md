...
邮件顶部新增期刊索引，列出本期有更新的期刊及文章数，方便在多期刊邮件中快速跳转和扫读。无摘要提示也从逐篇显示改为期刊分区下方统一提示，避免 ScienceDirect 期刊大量无摘要文章时重复出现同一句提示。

同次修正 RSS 标题中的 HTML 标签裸露问题。标题、作者和摘要在转义前会先去掉 `<em>`、`<i>` 等标签，避免邮件里显示 `<em>...</em>` 这类原始标记。

作者行随后取消斜体弱化效果，改为更深颜色和中等字重，使其在标题和摘要之间更容易被看到。

邮件抬头区随后恢复为无框的简洁标题区，只保留期次统计和阅读说明。顶部期刊索引被移除，避免多期刊邮件在正文开始前显得拥挤；期刊分区标题进一步加大加粗，使用更粗的顶部规则线强化期刊边界。

文章条目最后移除数字序号，标题、作者、日期和摘要直接与期刊标题左边缘对齐。文章之间的浅色分割线也同步去掉，只保留期刊分区标题的规则线，使正文更接近连续目录式阅读。

邮件抬头区（标题、期次统计、阅读说明）下方原有一条浅色横线（`border-bottom:1px solid #e5e7eb`）随后也一并删除，四个脚本同步。顶部区域与第一个期刊分区之间由间距自然分隔，不再依赖横线。
