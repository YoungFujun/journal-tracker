...
用法：
  正常运行（增量，更新缓存）:  python shangyin.py
  测试运行（全量，不写缓存）:  python shangyin.py --test
"""
...
SEEN_FILE        = STATE_DIR / "seen_shangyin.json"
FAIL_COUNTS_FILE = STATE_DIR / "fail_counts_shangyin.json"
LAST_SEEN_BY_JOURNAL_FILE = STATE_DIR / "last_seen_by_journal_shangyin.json"
SMTP_HOST        = "smtp.163.com"
SMTP_PORT        = 465
SENDER           = os.environ["EMAIL_SENDER"]
PASSWORD         = os.environ["EMAIL_PASSWORD"]
RECIPIENT        = os.environ["EMAIL_RECIPIENT_SHANGYIN"]
ALERT_RECIPIENT  = os.environ.get("EMAIL_ALERT", "")
FAIL_THRESHOLD   = 5
SCRIPT_NAME      = "shangyin"
...
# ── 抓取 CrossRef ─────────────────────────────────────────────────────────────
def fetch_crossref(seen: set, exclude_names=None) -> tuple:
    results, errors = {}, {}
...
def main(shared_top5_articles=None, shared_top5_article_errors=None, shared_top5_issues=None):
    ...
    print(f"=== shangyin tracker · {week_str} · 第{issue_num}期 · {mode_label} ===")
    ...
    articles, rss_errors    = fetch_rss(seen, exclude_names=exclude_names)
    cr_results, cr_errors   = fetch_crossref(seen, exclude_names=exclude_names)
    ...
    articles.update(top5_tracker.select_new_articles(shared_top5_articles, seen, allowed_names=TOP5_JOURNAL_NAMES))
