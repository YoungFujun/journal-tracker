...
日常周报由 `run_all_trackers.py` 统一调度。五大刊（AER、QJE、JPE、Econometrica、REStud）的近期新文章抓取和最新一期目录抓取会在每次运行时先统一执行一次，再分别对照各脚本自己的缓存文件判断“是否已推送”。这样既保留了各脚本独立的收件人、缓存和发信逻辑，也避免对五大刊重复请求。
...
弹出面板中可勾选要运行的脚本（默认全选），点击 **Run workflow** 触发。工作流会先统一抓取五大刊公共内容，再按你勾选的脚本分别生成和发送邮件。约 30 秒后检查邮箱（注意垃圾邮件文件夹）。收到邮件即部署成功。
