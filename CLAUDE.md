...
├── huang.py                  # 【可选预设】经济/金融/经济史 20 个期刊（社会学/政治学/PNAS 7 个暂停）
...
1. **更新 CHANGELOG.md**：将本次改动追加到 `CHANGELOG.md` 顶部（格式：`## YYYY-MM-DD` + `### 标题` + 说明段落）
2. **更新 README.md 更新记录**：将本次改动的一句话摘要追加到 `README.md` ## 更新记录 章节顶部（格式：`- **YYYY-MM-DD**：...`）；**所有改动均须执行，不限于功能性变更**
3. **同步文字描述**：凡涉及期刊数量、领域、功能的改动，还须同步更新以下位置的对应描述：
