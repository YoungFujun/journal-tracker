...
# ── CrossRef期刊列表（通过CrossRef API获取；包含无RSS期刊及OUP期刊）──────────
...
def build_html(new_articles: dict, week_str: str, issue_sections: dict = None) -> str:
...
<body style="margin:0; padding:0; background:#ffffff; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif;">
  <div style="max-width:760px; margin:0 auto; padding:34px 42px 28px 42px; background:#ffffff;">
    <div style="padding:0 0 26px 0; margin-bottom:34px;">
      ...
      <p style="color:#4b5563; margin:8px 0 0; font-size:16px; line-height:1.45;">{week_str} · {total} new articles across {len(new_articles)} journals{issue_note}</p>
...
def main(shared_top5_articles=None, shared_top5_article_errors=None, shared_top5_issues=None):
    ...
    html = build_html(new_articles, week_str, issue_sections=issue_sections)
